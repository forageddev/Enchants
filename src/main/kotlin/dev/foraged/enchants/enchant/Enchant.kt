...
abstract class Enchant(
    ...
    var tickHeld: Boolean = false,
    var tickWorn: Boolean = false
) : Listener
{
    companion object {
    ...
    fun apply(item: ItemStack, addLevel: Int) {
        ...
        val string = getFormattedLoreName() + " " +  (level)
        var updated = false
        ...
        if (this is VanillaOverride) {
            if (level > 32000) level = 32000
            meta.addEnchant(override, level, true)
